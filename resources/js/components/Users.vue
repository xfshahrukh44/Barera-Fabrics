            ...
            <div class="card">
              <div class="card-header">
                <h3 class="card-title mt-2"></h3>
                  <button class="btn btn-success xs" id="add_btn" @click="AddUserModal">
    ...
    export default {
        ...
        methods:{
          getResults(page = 1) {
            ...
            axios.get('garment/api/finduser?q='+query+'&page=' + page)
              .then(response => {
          ...
          deleteUser(id){
            Swal.fire({
              ...
              confirmButtonText: 'Yes, delete it!'
                ...
                if (result.value) {
                  this.form.delete('garment/api/user/'+id);
                  Swal.fire(
                    'Deleted!',
          ...
          loadUser(){
            axios.get('garment/api/user').then(({data})=>(this.users = data))
          },
          createUser(){
            this.$Progress.start();
            this.form.post('garment/api/user')
            .then(()=>{
          ...
          updateUser(){
            this.$Progress.start();
            this.form.put('garment/api/user/'+this.form.id)
            .then(()=>{
              Fire.$emit('searching');
              $('#userModal').modal('hide');
              Toast.fire({
        ...
        mounted() {
            ...
            Fire.$on('searching',()=>{
              let query = this.search;
              axios.get('garment/api/finduser?q='+query).then(({data})=>(this.users = data))
            });
